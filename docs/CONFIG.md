...
`milvus-onprem init` on the bootstrap VM, gitignored, and shared
identically across all peers (distributed automatically when each
new peer runs `join`).
...
| `MINIO_SECRET_KEY` | Yes | auto-generated by `init` | 24-char base64. Distributed by `join`. |
...
| `MINIO_HA_POOL_SIZE` | No | unset | Distributed only, integer ≥ 2. Set by `init --ha-cluster-size=N`. The first N peers form ONE erasure-coded pool that tolerates loss of any single host. Frozen at init; switching layouts later requires teardown + re-init. Unset = legacy per-host pools (no host-loss tolerance). See [FAILOVER.md § MinIO pool layout](FAILOVER.md#minio-pool-layout). |
